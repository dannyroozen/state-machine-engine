...
func TestEvaluator_Evaluate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "input equals OR true with repeated prefix",
			condition: "input_equals:input:status=accepted || input:status=reviewed",
			input:     []byte(`{"status":"reviewed"}`),
			want:      true,
			wantErr:   false,
		},
		{
			name:      "input equals OR false",
			condition: "input_equals:status=accepted || status=reviewed",
			input:     []byte(`{"status":"rejected"}`),
			want:      false,
			wantErr:   false,
		},
		{
			name:      "input equals AND true",
			condition: "input_equals:status=accepted && priority=high",
			input:     []byte(`{"status":"accepted","priority":"high"}`),
			want:      true,
			wantErr:   false,
		},
		{
			name:      "input equals AND false",
			condition: "input_equals:status=accepted && priority=high",
			input:     []byte(`{"status":"accepted","priority":"low"}`),
			want:      false,
			wantErr:   false,
		},
		{
			name:      "input equals mixed precedence",
			condition: "input_equals:status=accepted || status=reviewed && priority=high",
			input:     []byte(`{"status":"reviewed","priority":"high"}`),
			want:      true,
			wantErr:   false,
		},
		{
			name:      "input equals malformed logical expression",
			condition: "input_equals:status=accepted || ",
			input:     []byte(`{"status":"approved"}`),
			want:      false,
			wantErr:   true,
		},
		{
			name:      "input equals malformed OR expression left side true",
			condition: "input_equals:status=accepted || ",
			input:     []byte(`{"status":"accepted"}`),
			want:      true,
			wantErr:   false,
		},
		{
